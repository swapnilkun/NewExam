...
{	
	...
	$query_update_notice = "update notice set noitce='{$notice_textarea}', notice_subject='{$notice_subject}', notice_date='{$notice_date_from}' where n_id='".$_POST['n_id']."' ";
		...
		if($result_notice)
		{
			$message_success .=constant('TI_NOTICE_INSTRUCTION_SUCCESS_MESSAGE') ;;
		}
		else
		{
			$error .= constant('TI_INSTRUCTION_EDITED_FAILED_MESSAGE') ;
		}
		...
		<div class="row-fluid">
			<div class="area-top clearfix">
				<div class="pull-left header">
					<h3 class="title"><i class="icon-edit"></i><?php echo constant('TI_MANAGE_INSTRUCTION');?> </h3>
				</div>
			</div>
		</div>
	</div>
		...
		<div class="box">
			<div class="box-header">    	
				<ul class="nav nav-tabs nav-tabs-left">
					<li class="active">
						<a href="#add" data-toggle="tab"><i class="icon-wrench"></i><?php echo constant('TI_INSTRUCTION_EDIT');?></a>
					</li>
				</ul>	
			</div>
					...
					<form action="<?php $PHP_SELF ?>" method="post" accept-charset="utf-8" class="form-horizontal validatable" target="_top" enctype="multipart/form-data">
			         ...
			         <div class="padded"> 
                                ...
                                <div class="controls">
                                     <div class="box closable-chat-box">
                                        <div class="box-content">
                                                ...
                                                <textarea name="notice_textarea" id="ttt" class="validate[required]" rows="5" placeholder="add instruction"><?php echo $result['noitce'];?></textarea>
                                                </div>
                                        </div>
                                    </div>
                                </div>
						...
						<div class="form-actions">
							<button type="submit" class="btn btn-gray"><?php echo 'Edit Instruction';?></button>
