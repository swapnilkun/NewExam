...
{ 
	...
	else
	{
		...
		$query_insert_exam = "INSERT INTO exam (category_id,subcategory_id,subject_id,center_id,exam_name,exam_date,exam_time,exam_duration,neg_mark_status,negative_marks,passing_percentage,re_exam_day,terms_condition,result_show_on_mail,show_question,sort_order,time_reduction) VALUES('{$category_id}','{$subcategory_id}','{$subject_id}','".$_SESSION['center_id']."','{$exam_name}','{$exam_date_from}','{$exam_time}','{$exam_duration}','{$neg_mark_status}','{$negative_marks}','{$passing_percentage}','{$re_exam_day}','{$terms_condition}','{$result_show_on_mail}','{$show_question}','{$sort_order}','{$time_reduction}')";
                ...
                <table cellpadding="0" cellspacing="0" border="0" class="dTable responsive">
                    ...
                    <tbody>
							<?php 
								...
								$query=mysqli_query($con,"select * from exam where center_id= '".$_SESSION['center_id']."'");
								$i=0;
							...
							?>
								...
								<td><?php echo $exam_date_format;?> </td>
							...
							<div class="control-group">
                                ...
                                <div class="controls">
									...
									<option><?php echo constant('TI_SELECT_CATEGORY');?></option>  
										<?php $query_category=mysqli_query($con,"SELECT * FROM category c JOIN teacher t ON c.center_id=t.center_id WHERE category_status=1");
