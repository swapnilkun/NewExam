...
function getXMLHTTP() { 
	...
	function getstudent(category_id) {		
		
		var strURL="student_find.php?category_id="+category_id;
		var req = getXMLHTTP();
...
{ 
	...
	$query_center_result=mysqli_fetch_array(mysqli_query($con,"select * from teacher where center_id='".$_SESSION['center_id']."'"));	      
	...
	if($_POST['cbg1']!="")
	{
		...
		for ($i=0; $i<sizeof($checkBox); $i++)
		{
			...
			$message_success="";
			$query_student=mysqli_query($con,"select * from student where student_id='".$checkBox[$i]."'");
			$result_student=mysqli_fetch_array($query_student);
			$to=$result_student['student_email'];
			$student_name=$result_student['student_name'];	
			
			$messages="<html><body>
			<table>
			<tr><td>Hi ".$student_name.",</td></tr>	
			<tr><td>&nbsp;</td></tr>
			<tr><td>".$message."</td></tr>
			<tr><td>&nbsp;</td></tr>									
			<tr><td><b>Best Regards,</b></td></tr>
			<tr><td>".$_SESSION['teacher_name']."</td></tr>
			<tr><td>".$query_center_result['phone_no']."</td></tr>
			</table>
			</body></html>";
		...
		<ul class="nav nav-tabs nav-tabs-left">
			<li class="active">
            	<a href="#list" data-toggle="tab"><i class="icon-align-justify"></i> 
					<?php echo constant('TI_LABEL_CONTACT_FORM');?></a></li>
				<li>
				<a href="#add" data-toggle="tab"><i class="icon-plus"></i>
				<?php echo constant('TI_ADMIN');?>              	</a></li>
		</ul>
	...
	<div class="box-content padded">
		<div class="tab-content">
				  ...
				  <form action="<?php $PHP_SELF ?>" method="post" accept-charset="utf-8" class="form-horizontal validatable" target="_top" id="chk_option" onsubmit="return confirm_update();">
						...
						<div class="padded">       
							<div class="control-group">
                                ...
                                <div class="controls">
									<select name="category_id" class="validate[required]" onChange="getstudent(this.value)">
									<option value=""><?php echo constant('TI_SELECT_CATEGORY');?></option>  
			...
			<div class="tab-pane box" id="add" style="padding: 5px">
               <div class="box-content">
                   <form action="<?php $PHP_SELF ?>" method="post" accept-charset="utf-8" class="form-horizontal validatable" target="_top" id="chk_option_admin" onsubmit="return confirm_update_admin();">
...
<script type="text/javascript">
    function confirm_update() {
        ...
        for(i=0;i<aCheckbox.length;++i)
        {
         ...
         if(check==true)
         {
            ...
            {alert("Please select at least one studemt to send the mail");
            return false;
            }
	...
	<script type="text/javascript">
    function confirm_update_admin() {
        check=false;
        var aCheckbox = document.getElementById('chk_option_admin').elements;
        for(i=0;i<aCheckbox.length;++i)
        {
            if(aCheckbox[i].type=='checkbox' && aCheckbox[i].checked)
           {
