...
if($_GET['exam_id']!="")
{
	...
	if(in_array(decrypt_string($_GET['caid']),$center_id_explode))
	{
		$center_id=decrypt_string($_GET['caid']);
		$student_id=decrypt_string($_GET['sid']);
		$query_student_name=mysqli_fetch_array(mysqli_query($con,"select * from student where student_id='".$student_id."'"));
...
else
{
		 ...
		 $query1 = " SELECT max(exam_date) FROM practice_exam_status WHERE exam_id ='".$e_id."' AND category_id ='".$center_id."' AND student_id ='".$student_id."'";
...
$query_student_father=mysqli_fetch_array(mysqli_query($con,"select * from student where student_id='".$student_id."'"));// father name

$query_exam_take_status=mysqli_fetch_array(mysqli_query($con,"select * from practice_exam_status where exam_id='".$e_id."' and category_id='".$exam_name['category_id']."' and subcategory_id='".$exam_name['subcategory_id']."' and subject_id='".$exam_name['subject_id']."' and student_id='".$student_id."' and exam_date='".$given_exam_date."'"));

$subcategory_id= $exam_name['subcategory_id'];
$subject_id= $exam_name['subject_id'];

$query_answer=mysqli_query($con,"select * from practice_answer where student_id='".$student_id."' and exam_id='".$e_id."' and category_id='".$center_id."' and subcategory_id='".$exam_name['subcategory_id']."' and subject_id='".$exam_name['subject_id']."' and examdate='".$given_exam_date."' and practice_exam_status_id='".$query_exam_take_status['id']."'");

$count_queestion=mysqli_num_rows($query_answer);
//echo $count_queestion;
...
$query_status_count=mysqli_query($con,"select * from practice_answer where student_id='".$student_id."' and exam_id='".$e_id."' and category_id='".$exam_name['category_id']."' and subcategory_id='".$exam_name['subcategory_id']."' and 	subject_id='".$exam_name['subject_id']."' and examdate='".$given_exam_date."' and practice_exam_status_id='".$query_exam_take_status['id']."'");
...
else
{
	...
	$message.="<td class='news-title'>".ucfirst($query_student_name['student_name'])."</td>";
	$message.="</tr>";
	$message.="<tr>";
...
$query_answer_table=mysqli_query($con,"select * from practice_answer where student_id='".$student_id."' and exam_id='".$e_id."' and category_id='".$center_id."' and subcategory_id='".$exam_name['subcategory_id']."' and 	subject_id='".$exam_name['subject_id']."' and examdate='".$given_exam_date."' and practice_exam_status_id='".$query_exam_take_status['id']."'");
	$i=1;
	while($questin_result=mysqli_fetch_array($query_answer_table))
	{
		...
		{			
			...
			if($questin_result['typeofquestion']=='Single')
			{
				...
				$message.="<td  class='news-title'>".$correct_ans_single."</td>";
			}
			else
			{
				$message.="<td  class='news-title'>".$correct_ans_multi."</td>";
			}
