...
//	$student_photo_id= mysql_prep($_POST['student_photo_id']);
	$student_photo_id_desc= mysql_prep($_POST['student_photo_id_desc']);
		...
		$query1 = "delete from installment  where student_id='{$_POST['student_id']}'";
		mysqli_query($con,$query1);
	
	foreach ($_POST['amtrecieved'] as $row=>$amtrecieved) {
...
$query=mysqli_query($con,"select * from student s,installment i where s.student_id='$student_id'");
				  ...
				  <div id="tab_3">		
					...
					<table class="table table-bordered">
								...
								<tbody id="courses-rows">
									...
									$i=0;
								//	$student_id=$_GET['student_id'];
                                    $query=mysqli_query($con,"select * from student s,installment i where s.student_id='$student_id' order by i.id");
                                   while($res=mysqli_fetch_array($query)){
									...
									<tr id="courserow-<?php echo $i;?>">
							        	   ...
							        	   <td>
								        	<select class="form-control course" name="course[]" id="course<?php echo $i;?>" onchange="getInstCourseFees(this.value, this.id)" readonly>
				     
										     <?php 
                        								   $query_batch=mysqli_query($con,"select subject_name from subject");
                        								    while($result_batch=mysqli_fetch_array($query_batch))
                        								   {?>
                        								 <option value="<?php echo $result_batch['subject_name'];?>" <?php if($result_batch['subject_name']== $res['course_name']) { echo "selected=selected"; }?>> <?php echo $result_batch['subject_name']; ?></option>
											    	<?php }
                        						    ?>
                        					        </select>	
											...
											<td>
											    ...
											    /*$student_id=$_GET['student_id'];
                                                    ...
                                                    ?>
                                                	<input type="hidden" class="form-control id" name="id[]" id="id<?php echo $i;?>"  value="<?php echo $res['id']; ?>"/>
												   
											   	<input type="text" class="form-control coursefees" name="coursefees[]" id="coursefees<?php echo $i;?>"  value="<?php echo $res['course_fee']; ?>"/ readonly>
												
												
												</td>
											<td>
											
										<select class="form-control discrate" name="discrate[]" id="discrate<?php echo $i;?>" onchange="calDiscountedAmt(<?php echo $i;?>)" readonly>
												 <option value="" >Select---</option>
												   <option value="amtminus" <?php if($res['discount_rate'] == 'amtminus'){ echo "selected=selected"; } ?>>Amount - </option>
                                                    <option value="amtplus" <?php if($res['discount_rate'] == 'amtplus') { echo "selected=selected"; } ?>>Amount +</option>
                                                    <option value="perminus" <?php if($res['discount_rate'] == 'perminus') { echo "selected=selected"; } ?>>% - </option>
                                                    <option value="perplus" <?php if($res['discount_rate'] == 'perplus') { echo "selected=selected"; } ?>>% +</option>
                                               	</select>
											</td>

											<td>	
																						
											<input type="text" class="form-control" name="discamt[]" id="discamt<?php echo $i;?>" onchange="calDiscountedAmt(<?php echo $i;?>)" onkeyup="calDiscountedAmt(<?php echo $i;?>)" / readonly>
											</td>
											<td>	
																						
											<input type="text" class="form-control" name="install[]" id="install<?php echo $i;?>" onchange="install(<?php echo $i;?>)" onkeyup="install(<?php echo $i;?>)" value="<?php echo $res['installment']; ?>" readonly/>
											</td>
											<td>	
																						
											<input type="date" class="form-control" name="install_date[]" id="install_date<?php echo $i;?>"  value="<?php echo $res['installment_date']; ?>"/ readonly>
											</td>
											
											<td>
												
											<input type="text" class="form-control" name="totalcoursefee[]" id="totalcoursefee<?php echo $i;?>" value="<?php echo $res['total_fee']; ?>" readonly></td>
											<td>	
																							
											<input type="text" class="form-control" name="amtrecieved[]" id="amtrecieved<?php echo $i;?>" onchange="calTotalPerCourse(<?php echo $i;?>)" onkeyup="calTotalPerCourse(<?php echo $i;?>)" value="<?php echo $res['fee_received']; ?>"/ readonly>
											<span style="color:#f00" id="amtrecieved_err<?php echo $i;?>"></span>
											</td>
											<td>	
																						
											<input type="text" class="form-control amtbalance" name="amtbalance[]" id="amtbalance<?php echo $i;?>" value="<?php echo $res['balance']; ?>" readonly /></td>
											<td>
												
											<textarea class="form-control" name="payremarks[]" id="payremarks<?php echo $i;?>" ><?php echo $res['remark'];?></textarea></td>
											
											<td><a href="javascript:void(0)" onclick="deleteCourseRow(<?php echo $i;?>)" class="btn btn-xs btn-danger"><i class="fa fa-minus-circle"></i></a></td>
										</tr>
										<?php
                                            }
                                         ?>
								...
								<tfoot>
									<tr>
										...
										<td colspan="10">
										<input type="hidden" name="countcourses" id="countcourses"  value="<?php echo $i;?>" />
										</td>
    ...
    function addCourseRow()
{
	var lastRowIndex = parseInt($("#countcourses").val())+1;
		var id = parseInt($("#countcourses").val());
	var balance = parseInt($("#amtbalance"+id).val());
	var coursefees = parseInt($(".coursefees").val());
	var course = $(".course").val();
 	var discrate = $(".discrate").val();
	//alert(id);	
	
	$.post('add_installment.php',
   	{
	    action:'add_new_course_row',lastrowindex:lastRowIndex,balance:balance,coursefees:coursefees,course:course,discrate:discrate}, 
